...
public class AddOnResource {
    ...
    @Path("retrieveAddOn/{addOnId}")
    @GET
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.APPLICATION_JSON)
    public Response retrieveAddOn(@PathParam("addOnId") Long addOnId) {
        try {
            AddOn addOnEntity = addOnSessionBeanLocal.retrieveAddOnByAddOnId(addOnId);
            
            return Response.status(Response.Status.OK).entity(addOnEntity).build();
        } catch (Exception ex) {
